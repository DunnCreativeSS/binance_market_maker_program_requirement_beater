    ...
    def process_message(self, msg):
        try: 
            ...
            if data['e'] == 'ORDER_TRADE_UPDATE':
                ...
                if type != 'NEW':
                    #print(type)
                    if data['o']['s'].replace('USDT', '/USDT') in self.openorders:
                        for o in self.openorders[data['o']['s'].replace('USDT', '/USDT')]:
                            if o['id'] == data['o']['i']:
                                self.openorders[data['o']['s'].replace('USDT', '/USDT')].remove(o)
                else:
                    
                    try:
                        self.openorders[data['o']['s'].replace('USDT', '/USDT')].append({'id':data['o']['i'], 'type': type, 'datetime': data['o']['T'], 'price': price, 'qty': qty, 'side': side})
                    except:
                        self.openorders[data['o']['s'].replace('USDT', '/USDT')] = []
                        self.openorders[data['o']['s'].replace('USDT', '/USDT')].append({'id':data['o']['i'], 'type': type, 'datetime': data['o']['T'], 'price': price, 'qty': qty, 'side': side})
                
                if type == 'TRADE':
                    try:
                        self.trades[data['o']['s'].replace('USDT', '/USDT')].append({'id':data['o']['i'], 'type': type, 'datetime': data['o']['T'], 'price': price, 'qty': qty, 'side': side})
                    except:
                        self.trades[data['o']['s'].replace('USDT', '/USDT')] = []
                        self.trades[data['o']['s'].replace('USDT', '/USDT')].append({'id':data['o']['i'], 'type': type, 'datetime': data['o']['T'], 'price': price, 'qty': qty, 'side': side})
